...
public class Bookcase<T> implements IProduct<T> {
	private String productName;
	public Bookcase(String productName) {//constructor
		this.productName = productName;
	}
	@SuppressWarnings({ "rawtypes", "unchecked" })
	public boolean isStored(T warehouse) {//It checks the element if it is stored in warehouse 
		
		return (((BookcaseWarehouse) warehouse).contains("Bookcase"));
	}
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public boolean isManufactured(T factoryLine) {
	    
	    if(((factoryLine) factoryLine).contains("Bookcase")) {
	        return true;
	    }
	    else {
	    	return false;
